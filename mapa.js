...
async function initMap() {
  ...
  // NUEVO: Live Tracking Layer
  window.liveTrackingLayer = L.layerGroup().addTo(map);
...
// ========= LIVE TRACKING (NUEVO) =========
const liveMarkersMap = {}; // userId -> L.marker

function startLiveTracking() {
  console.log("[Live] Starting listener...");

  // 1. Initial Fetch of active trackers
  ...
  // 3. Periodic cleanup/refresh (every 60s)
  setInterval(activeTrackerFetcher, 60000);
}
...
function handleLiveUpdate(user) {
  ...
  if (now - lastSeen > MAX_AGE) {
    const m = liveMarkersMap[user.id];
    if (m) {
  ...
  // divIcon is HTML based.
  const icon = L.divIcon({
    className: 'live-marker',
    html: `<div style="font-size:24px; text-shadow:0 2px 4px rgba(0,0,0,0.3); display:flex; justify-content:center; align-items:center; width:100%; height:100%;">${emoji}</div>`,
    iconSize: [30, 30],
    iconAnchor: [15, 15],
    popupAnchor: [0, -10]
  });
  ...
  if (liveMarkersMap[user.id]) {
    const marker = liveMarkersMap[user.id];
    marker.setLatLng(latlng);
    marker.setIcon(icon);

    const popupContent = `
            <b>${escapeHtml(user.nombre)}</b><br>
            <span class="tag">${escapeHtml(user.role)}</span><br>
            <small>Hace: ${timeSince(user.last_seen)}</small>
        `;
    marker.getPopup().setContent(popupContent);

  } else {
    const marker = L.marker(latlng, { icon: icon }).addTo(window.liveTrackingLayer);
