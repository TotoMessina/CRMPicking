...
async function main() {
    ...
    console.log("Checking columns in 'actividades' table...");
    const { data, error } = await supabase.from('actividades').select('*').limit(1);

    if (error) {
        console.error("Error:", error.message);
    } else if (data && data.length > 0) {
        console.log("Found keys in 'actividades':", Object.keys(data[0]));
    } else {
