...
BEGIN
    ...
    WHERE ec.empresa_id = p_empresa_id
      ...
      AND (
          (p_estado IS NULL AND p_estados IS NULL) OR
          (p_estado IS NOT NULL AND ec.estado = p_estado) OR
          (p_estados IS NOT NULL AND ec.estado = ANY(p_estados))
      )
      AND (
          (p_situacion IS NULL AND p_situaciones IS NULL) OR
          (p_situacion IS NOT NULL AND ec.situacion = p_situacion) OR
          (p_situaciones IS NOT NULL AND ec.situacion = ANY(p_situaciones))
      )
      AND (
          (p_tipo_contacto IS NULL AND p_tipos_contacto IS NULL) OR
          (p_tipo_contacto IS NOT NULL AND ec.tipo_contacto = p_tipo_contacto) OR
          (p_tipos_contacto IS NOT NULL AND ec.tipo_contacto = ANY(p_tipos_contacto))
      )
      ...
      AND (
          (p_rubro IS NULL AND p_rubros IS NULL) OR
          (p_rubro IS NOT NULL AND ec.rubro = p_rubro) OR
          (p_rubros IS NOT NULL AND ec.rubro = ANY(p_rubros))
      )
      AND (
          (p_interes IS NULL AND p_intereses IS NULL) OR
          (p_interes IS NOT NULL AND ec.interes = p_interes) OR
          (p_intereses IS NOT NULL AND ec.interes = ANY(p_intereses))
      )
      AND (
          (p_estilo IS NULL AND p_estilos IS NULL) OR
          (p_estilo IS NOT NULL AND ec.estilo_contacto = p_estilo) OR
          (p_estilos IS NOT NULL AND ec.estilo_contacto = ANY(p_estilos))
      )
