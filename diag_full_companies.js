import { createClient } from '@supabase/supabase-js';

const SUPABASE_URL = "https://mflftikcvsnniwwanrkj.supabase.co";
const SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1mbGZ0aWtjdnNubml3d2FucmtqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjM1NjcyMjAsImV4cCI6MjA3OTE0MzIyMH0.Z_EsaegFay24E0rOoX2PpwvWasWm5tfLcJiRrgs1nBY";

const supabase = createClient(SUPABASE_URL, SUPABASE_KEY);

async function main() {
    console.log("Logging in as Super Admin...");
    await supabase.auth.signInWithPassword({
        email: 'lucastotomessina@gmail.com',
        password: 'Toto2003'
    });

    console.log("Listing all companies...");
    const { data: companies, error: compError } = await supabase.from('empresas').select('*');
    if (compError) console.error("Error companies:", compError.message);
    else console.log("Companies:", companies);

    console.log("Checking consumers with empresa_id = null...");
    const { data: consumers, error: consError, count } = await supabase
        .from('consumidores')
        .select('*', { count: 'exact' })
        .is('empresa_id', null);

    if (consError) console.error("Error consumers:", consError.message);
    else console.log("Found", count, "consumers without empresa_id.");
}

main();
