...
interface MapPoint {
    ...
    lat: number;
    lng: number;
    tipo: 'cliente' | 'consumidor' | 'repartidor';
}
...
export default function MapaGlobal() {
    ...
    const [data, setData] = useState<{
        clientes: MapPoint[];
        consumidores: MapPoint[];
        repartidores: MapPoint[];
    }>({
        clientes: [],
        consumidores: [],
        repartidores: []
    ...
    const applyJitter = (lat: number, lng: number): [number, number] => {
        const spread = 0.0004; 
        return [
    ...
    const fetchData = async () => {
        ...
        try {
            // 0. Fetch Absolute Totals independently (Resilient loading)
             const fetchCount = async (table: 'consumidores' | 'repartidores', isActive = false) => {
                try {
            ...
            setAvailableUsers((usersInfo as AvailableUser[]) || []);
    ...
    const fetchHistory = async () => {
        ...
        try {
            ...
            if (error) throw error;
            setHistoryData((hist as unknown as HistoryPoint[]) || []);
    ...
    useEffect(() => {
        ...
        if (historyMode && historyData.length > 1) {
            const latlngs: L.LatLngExpression[] = historyData.map(p => [p.lat, p.lng]);
    ...
    const toggleVisibility = (cat: 'clientes' | 'consumidores' | 'repartidores') => {
    ...
    return (
        <div className="map-immersive-container">
            ...
            <div className="map-main-view">
                ...
                <MapStatsBadge inView={0} total={0}>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: isMobile ? '10px' : '20px', textAlign: 'center' }}>
                        <div>
                            <div style={{ fontSize: '0.65rem', color: CATEGORY_COLORS.clientes, fontWeight: 800, textTransform: 'uppercase' as const }}>{t('map_global.layers.clientes')}</div>
                        ...
                        <div style={{ borderLeft: '1px solid var(--border)', paddingLeft: isMobile ? '10px' : '20px' }}>
                            <div style={{ fontSize: '0.65rem', color: CATEGORY_COLORS.consumidores, fontWeight: 800, textTransform: 'uppercase' as const }}>{t('map_global.layers.consum_short')}</div>
                            <div style={{ fontSize: '1rem', fontWeight: 800 }}>{totals.consumidores}</div>
                        </div>
                        <div style={{ borderLeft: '1px solid var(--border)', paddingLeft: isMobile ? '10px' : '20px' }}>
                            <div style={{ fontSize: '0.65rem', color: CATEGORY_COLORS.repartidores, fontWeight: 800, textTransform: 'uppercase' as const }}>{t('map_global.layers.repart_short')}</div>
                ...
                <MapLegend 
                    ...
                    items={Object.entries(CATEGORY_COLORS).map(([key, color]) => ({ 
                        label: `${t(`map_global.layers.${key}`)} (${countsInZone[key as keyof typeof countsInZone]})`, 
                        color 
                    }))}
                    isMobile={isMobile}
