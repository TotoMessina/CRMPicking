...
export default function Usuarios() {
    ...
    const [rolesDisponibles, setRolesDisponibles] = useState<CrmRol[]>([]);
    ...
    const fetchRoles = async () => {
        ...
        try {
            ...
            if (!error && data) {
                setRolesDisponibles(data as CrmRol[]);
            }
        } catch (err) {
    ...
    const fetchUsuarios = async () => {
        ...
        try {
            ...
            const merged = (usersData || []).map(u => {
                const eu = euData.find((e: any) => e.usuario_email === u.email);
                return {
                    ...u,
            ...
            setUsuarios(merged as UsuarioEmpresa[]);
        } catch (error) {
    ...
    const handleOpenModal = (user: UsuarioEmpresa) => {
    ...
    const handleSave = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!selectedUser || !empresaActiva?.id) return;
                        ...
                        <form onSubmit={handleSave} style={{ padding: '24px' }}>
                            ...
                            <div className="field" style={{ marginBottom: '20px' }}>
                                ...
                                >
                                    ...
                                    {rolesDisponibles.length > 0 && <optgroup label={t('users.modal.role_custom', { company: empresaActiva?.nombre }) as string}>
                                        {rolesDisponibles.map(r => (
