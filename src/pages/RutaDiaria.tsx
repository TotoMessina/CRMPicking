...
const makeNumberedIcon = (num: number, color: string, done: boolean) => L.divIcon({
    className: '',
    iconSize: [32, 32],
    iconAnchor: [16, 32],
    popupAnchor: [0, -34],
...
interface Visita {
    ...
    estado: 'Pendiente' | 'Visitado' | 'Ausente' | 'Cancelado';
    orden: number;
...
export default function RutaDiaria() {
    ...
    const [vista, setVista] = useState<'lista' | 'mapa' | 'ambos'>('lista');
    ...
    const fetchVisitas = async () => {
        ...
        try {
            ...
            setVisitas(visitasRaw.map((v: any) => ({ ...v, clientes: clienteMap[v.cliente_id] || null })));
        } catch (e) {
            console.error(e);
    ...
    const marcarEstado = async (visitaId: string, nuevoEstado: Visita['estado']) => {
    ...
    const abrirMaps = (direccion?: string | null, lat?: number | null, lng?: number | null) => {
        const query = lat && lng ? `${lat},${lng}` : encodeURIComponent(direccion || '');
        window.open(`https://www.google.com/maps/dir/?api=1&destination=${query}`, '_blank');
    };

    const abrirWaze = (lat?: number | null, lng?: number | null, direccion?: string | null) => {
        if (lat && lng) {
    ...
    const polylinePoints: [number, number][] = puntosConCoords.map(v => [v.clientes!.lat!, v.clientes!.lng!]);
