...
import { 
    ...
    Calendar, Loader2, Info, Activity
} from 'lucide-react';
...
export default function Historial() {
    ...
    useEffect(() => {
        ...
        if (clientId && empresaActiva?.id) {
            handleSelectClient({ id: clientId, nombre_local: '' });
        }
    }, [empresaActiva]);

    const handleSearch = async (val: string) => {
        setSearchTerm(val);
        if (val.length < 2 || !empresaActiva?.id) {
            setSearchResults([]);
            return;
        }
        setSearching(true);
    ...
    const handleSelectClient = async (client: { id: string | number; nombre_local?: string }) => {
        ...
        try {
            ...
            const { data: ecData, error: ecError } = await supabase
                ...
                .eq('cliente_id', client.id as any)
                .eq('empresa_id', empresaActiva.id)
                .single();

            if (ecError) throw ecError;
            setSelectedClient(ecData as SelectedClientData);
            ...
            const consolidatedEvents: ConsolidatedEvent[] = [];

            // Process Status History (from ecData)
            const statusHistory = ecData.status_history;
            if (statusHistory && Array.isArray(statusHistory)) {
                statusHistory.forEach((sh: any) => {
            ...
            activitiesRes.data?.forEach(act => {
                ...
                consolidatedEvents.push({
                    ...
                    by: act.usuario || undefined,
                    details: act.user_id ? `ID Usuario: ${act.user_id}` : undefined
                });
            });
            ...
            eventsRes.data?.forEach(evt => {
                consolidatedEvents.push({
                    ...
                    details: evt.descripcion || undefined,
                    by: (evt.creado_por || evt.usuario) || undefined
                });
            });
            ...
            auditRes.data?.forEach(log => {
                consolidatedEvents.push({
                    ...
                    by: log.changed_by || undefined
                });
            });

            // Sort by date descending
            consolidatedEvents.sort((a, b) => new Date(b.at).getTime() - new Date(a.at).getTime());
