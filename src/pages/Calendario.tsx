...
import { EventoCalendarioModal, InitialEventData } from '../components/ui/EventoCalendarioModal';
...
export default function Calendario() {
    ...
    const loadUsuarios = async () => {
        ...
        try {
            ...
            if (usrs) {
                usrs.forEach((u: any) => u.usuarios?.nombre && set.add(u.usuarios.nombre));
            }
            ...
            if (cls) {
                cls.forEach((c: any) => c.clientes?.responsable && set.add(c.clientes.responsable));
            }
            ...
            const { data: acts } = await supabase.from('actividades')
                ...
                .not('usuario', 'is', null);
            if (acts) {
                acts.forEach((a: any) => a.usuario && set.add(a.usuario));
            }
        ...
        const loadEvents = async () => {
            try {
                ...
                if (filtroTipo === "todos" || filtroTipo === "internos") {
                    ...
                    let pivotMap = new Map<string, string[]>();
    ...
    const openCreateModal = (start?: Date | null, end?: Date | null) => {
        setIsContacto(false);
        setEditingId(null);
    ...
    const handleEventClick = (info: any) => {
        ...
        if (props.kind === "contacto") {
            setIsContacto(true);
            setEditingId(null);
            setEditingClienteId(Number(props.clienteId));
            setInitialData({
            ...
            setIsContacto(false);
            setEditingId(Number(props.dbId));
            setEditingClienteId(null);
            setInitialData({
