...
import { 
    ...
    Phone, Mail, Calendar, Clock, User, MessageSquare, MoreVertical, FileText, Edit2, Trash2 
} from 'lucide-react';
...
export default function Consumidores() {
    ...
    const [consumidores, setConsumidores] = useState<Consumer[]>([]);
    ...
    const fetchConsumidores = async () => {
        ...
        if (import.meta.env.DEV) console.log("Consumidores: Fetching for company:", empresaActiva.id);
        setLoading(true);
        ...
        if (error) {
            ...
            setConsumidores((data as unknown as Consumer[]) || []);
            setTotal(count || 0);

            // Fetch activities for these consumers
            if (data && data.length > 0) {
                const ids = (data.map(c => c.id) as unknown) as number[];
                ...
                if (!actError && acts) {
                     ...
                     acts.forEach(a => {
                         if (a.consumidor_id != null) {
                             const key = String(a.consumidor_id);
    ...
    const handleDelete = async (id: string | number) => {
        const confirmed = await askConfirm({
            title: t('consumers.delete_title', { defaultValue: 'Eliminar consumidor' }),
            message: t('consumers.confirm_delete'),
        ...
        const { error } = await supabase.from("consumidores").update({ activo: false }).eq("id", id as any);
        if (error) {
    ...
    return (
        <div className="page-container" style={{ padding: '0', maxWidth: '100%', margin: '0 auto', position: 'relative' }}>
            ...
            <section style={{ marginBottom: '32px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '20px' }}>
                    {loading ? (
                        ...
                        const hasAddress = Boolean(c.localidad);
                        ...
                        return (
                            <div key={c.id} className="bento-card" style={{ padding: '24px', position: 'relative', display: 'flex', flexDirection: 'column', gap: '16px', overflow: 'hidden' }}>
                                ...
                                {['Cliente', 'Interesado'].includes(c.estado) && <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '4px', background: c.estado === 'Cliente' ? 'var(--success)' : 'var(--accent)' }}></div>}
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                    <div style={{ flex: 1, paddingRight: '12px' }}>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', color: 'var(--text-muted)', fontSize: '0.95rem' }}>
                                            {hasAddress && <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><MapPin size={15} /> {c.localidad}</div>}
                                    ...
                                    <div style={{ display: 'flex', gap: '6px' }}>
                                        <button onClick={() => handleEdit(Number(c.id))} style={{ padding: '8px', borderRadius: '10px', background: 'var(--bg)', color: 'var(--text-muted)', border: '1px solid var(--border)', cursor: 'pointer' }} title={t('common.edit') as string}>
                                            <Edit2 size={16} />
                                        </button>
                                        {!isDemoMode && (
                                            <button onClick={() => handleDelete(c.id)} style={{ padding: '8px', borderRadius: '10px', border: '1px solid var(--border)', background: 'rgba(239, 68, 68, 0.05)', color: 'var(--danger)', cursor: 'pointer' }} title={t('common.delete') as string}>
                                                <Trash2 size={16} />
                                            </button>
                                        )}
                                    </div>
                                ...
                                <div style={{ display: 'flex', gap: '8px', marginTop: 'auto', paddingTop: '16px', borderTop: '1px dashed var(--border)' }}>
                                    <Button variant="primary" style={{ flex: 1, padding: '8px' }} onClick={() => handleOpenActivity(Number(c.id), c.nombre)}>
            ...
            <ActividadConsumidorModal
                isOpen={actModalOpen}
                onClose={() => setActModalOpen(false)}
                consumidorId={actTargetId || ''}
            ...
            {createPortal(
                <div style={{ position: 'fixed', bottom: '30px', right: '30px', zIndex: 9999 }}>
                    <motion.button
                        ...
                        title={t('consumers.new_consumer') as string}
                    >
                        <Plus size={32} />
                    </motion.button>
                </div>,
