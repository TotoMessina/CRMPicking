...
export default function PipelineSettings() {
    ...
    const [activeTab, setActiveTab] = useState<'states' | 'situations'>('states');

    // States logic
    const { states: initialStates, loading: statesLoading, refresh: refreshStates } = usePipelineStates(empresaActiva?.id);
    const [localStates, setLocalStates] = useState<PipelineItem[]>([]);
    ...
    const [localSituations, setLocalSituations] = useState<PipelineItem[]>([]);
    ...
    useEffect(() => {
        if (initialStates) setLocalStates([...initialStates] as PipelineItem[]);
    }, [initialStates]);

    useEffect(() => {
        if (initialSituations) setLocalSituations([...initialSituations] as PipelineItem[]);
    }, [initialSituations]);
    ...
    const handleUpdateItem = (id: string, field: keyof PipelineItem, value: any) => {
    ...
    const toggleStateVisibility = (itemId: string, stateLabel: string) => {
        setActiveItems(prev => prev.map(s => {
            if (s.id === itemId) {
    ...
    const handleRemoveItem = async (id: string, db_id?: string) => {
        ...
        const item = activeItems.find(s => s.id === id);
        if (!item) return;
        ...
        try {
            ...
            const toUpsert = itemsToSave.map(s => {
                ...
                const row: any = {
                    ...
                    label: s.label?.trim(),
                    color: s.color,
                    orden: typeof s.orden === 'string' ? parseInt(s.orden) : s.orden || 0,
                                                ...
                                                >
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 16px' }}>
                                                        ...
                                                        <input type="text" value={item.label} onChange={(e) => handleUpdateItem(item.id, 'label', e.target.value)}
                                                            className="input" placeholder={t('pipeline.settings.form.name_placeholder') as string} style={{ flex: 1, height: '38px', fontSize: '0.9rem', fontWeight: 700, border: 'none', background: 'transparent' }} />
