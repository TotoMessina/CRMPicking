...
interface ActividadConsumidorModalProps {
    isOpen: boolean;
    onClose: () => void;
    consumidorId: string | number;
    consumidorNombre: string;
    onSaved: () => void;
}

export function ActividadConsumidorModal({ 
    isOpen, 
    onClose, 
    consumidorId, 
    consumidorNombre, 
    onSaved 
}: ActividadConsumidorModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (error) {
            ...
            await supabase.from("consumidores").update({ ultima_actividad: fechaISO }).eq("id", Number(consumidorId));
        ...
        <div className="modal is-open" onClick={onClose}>
            <div className="modal-content modal-sm" onClick={e => e.stopPropagation()}>
                <div className="modal-header">
                    <h3>{t('consumers.activity.add_title')}</h3>
