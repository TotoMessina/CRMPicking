...
export function EventoCalendarioModal({ 
    ...
    const fetchHistorialContacto = async (cId: number) => {
    ...
    const setEventUsers = async (evId: number, users: string[]) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isContacto) {
                ...
                }).eq('cliente_id', clienteId as number).eq('empresa_id', empresaActiva?.id || '');
                ...
                if (eventoId) {
                    const { error } = await supabase.from("eventos").update(payload).eq("id", eventoId as number);
                    if (error) throw error;
                } else {
                ...
                if (savedEventId) {
                    await setEventUsers(savedEventId as number, formData.usuarios);
                }
    ...
    const handleDelete = async () => {
        ...
        setLoading(true);
        const { error } = await supabase.from("eventos").delete().eq("id", eventoId as number);
        if (error) {
        ...
        <div className="modal is-open" onClick={onClose}>
            <div className="modal-content" onClick={e => e.stopPropagation()}>
                <div className="modal-header">
                    <h3>{isContacto ? 'Editar Contacto' : (eventoId ? 'Editar Evento' : 'Nuevo Evento')}</h3>
